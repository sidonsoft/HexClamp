...
def parse_browser_action(text: str) -> dict:
    ...
    if "search" in text_lower:
        match = re.search(
            r'search\s+(?:for\s+)?["\']?([^"\']+)["\']?', text, re.IGNORECASE
        )
        if match:
...
def run_browser_task(task: dict, workdir: Path) -> dict:
    ...
    if not urls:
        ...
        if task.get("search_terms"):
            urls = [
                f"https://www.google.com/search?q={task['search_terms'].replace(' ', '+')}"
            ]
        else:
...
def main():
    ...
    parser.add_argument(
        "--workdir", required=True, help="Working directory for artifacts"
    )
