...
def find_pending_tasks(executor_filter: Optional[str] = None) -> list[dict]:
    ...
    executors = (
        ...
        if executor_filter is None
        else [executor_filter]
    )
...
def update_loop_state(task: dict, verification: dict, dry_run: bool = False):
    ...
    task_data = task["task"]
    event_id = task_data.get("event_id") or task_data.get("loop_id", "").replace(
        "loop-", ""
    )
    ...
    # Find matching loop
    for loop in loops:
        if loop.get("id") == f"loop-{event_id}" or loop.get("id") == task_data.get(
            "loop_id"
        ):
            ...
            if dry_run:
                print(
                    f"  [DRY-RUN] Would update loop {loop['id']}: {old_status} → {loop['status']}"
                )
            else:
...
def complete_task(task: dict, dry_run: bool = False):
    ...
    if not dry_run:
        ...
        task["task_file"].write_text(
            json.dumps(task["task"], indent=2), encoding="utf-8"
        )
...
def main():
    ...
    parser.add_argument(
        "--executor",
        choices=["code", "browser", "messaging"],
        help="Filter by executor type",
    )
