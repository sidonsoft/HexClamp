...
def classify_text(text: str) -> str:
    ...
    # Split patterns into multi-word vs single-word
    multi_word_system_patterns = [
        "pause loop",
        "set mode",
        "set state",
        "update config",
    ...
    # Check multi-word patterns first (exact phrase matching)
    if any(pattern in normalized for pattern in multi_word_system_patterns):
        return "system"
    
    # Context guard: check if text is short (<=3 words) OR starts with command prefix
    word_count = len(normalized.split())
    has_command_prefix = normalized.startswith(('please ', '!', '/', 'hey ', 'bot '))
    
    # Only classify as system if context guard passes (short text OR command prefix)
    if (word_count <= 3 or has_command_prefix):
        # For single-word patterns, use word boundaries to avoid false positives
        for pattern in single_word_system_patterns:
