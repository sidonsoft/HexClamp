...
def verify_result(action: Action, summary: str, evidence: List[str] | None = None, artifacts: List[str] | None = None) -> Result:
    ...
    if type_requires_verification:
        ...
        missing_files = [e for e in evidence if not _is_valid_evidence(e) and not any(e.startswith(p) for p in ("agent:", "git:", "quality_gate:", "syntax:", "py_compile:"))]
        if missing_files:
            count_ok = False
    ...
    result = Result(
        ...
        evidence=evidence,
        artifacts=artifacts,
        follow_up=[] if verified else [f"Need at least {minimum} valid evidence item(s) for {action.type}"],
        verified=verified,
    )
